...
export function CatalogueCategoryList() {
  ...
  useEffect(
    function () {
      async function getData() {
        setLoading(true);
        let url = `/catalogueCategories?page=${pagination.page}&limit=${pagination.limit}`;
          ...
          return (
            <div className="d-flex gap-1">
              <Link
                ...
                to={{
                  pathname: `/catalogueCategories/edit/${value}`,
                }}
              >
  ...
  async function handleDeleteData(recordId: string | string[]) {
    ...
    if (Array.isArray(recordId)) {
      apiResponse = await remove(`/catalogueCategories`, recordId);
    } else {
      apiResponse = await remove(`/catalogueCategories/${recordId}`);
    }
